...
function ensureRankLayer() {
  var scene = cc.director.getScene();
  var existing = findNodeByName(scene, "WechatFriendRankLayer");
  if (existing !== null) {
    return existing;
  }
  ...
  layer.parent = scene;
  layer.zIndex = 900;
  layer.setContentSize(DESIGN_WIDTH, DESIGN_HEIGHT);
  ...
  createTouchableNode("WechatFriendRankProgressTab", layer, -105, 326, 190, 70, function () {
    showRankLayer("progress");
  });
  createTouchableNode("WechatFriendRankTotalTab", layer, 105, 326, 190, 70, function () {
    showRankLayer("total");
  });
  createTouchableNode("WechatFriendRankClose", layer, 282, 438, 96, 96, hideRankLayer);
...
function install() {
  ...
  cc.director.getScheduler().schedule(function () {
    ...
    bindRankingButton();
  }, cc.director, 0.5, cc.macro.REPEAT_FOREVER, 0, false, "wechat_rank_install");
}
