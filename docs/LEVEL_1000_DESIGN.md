...
- After each fired bubble finishes its shot resolution, every pair shifts simultaneously. Only the strict interior slots between that pair's fixed endpoints move exactly one slot in `moveDirection`, wrapping through the two endpoints. Normal balls, special balls, vine ownership state and empty slots all move together; the wrapped ball shrinks and fades into the entry endpoint, then grows and fades out of the opposite endpoint before reaching its target slot.
- Wormhole endpoints never move with their own cycle and never participate in color matching. Each endpoint reserves its authored coordinate as `.`: neither level content nor runtime attachment may place a normal or special ball there. Endpoints remain outside `BubbleGrid.cells`, so they provide no support and do not contribute to the board's bottom pressure limit.
- Aim prediction and the real projectile path both test the same earliest wormhole collision. A fired ball cannot pass through a wormhole: it stops at the contact point, plays a shrink/fade suction into the endpoint center, disappears without attaching, consumes the shot normally, and still advances the post-shot special phases.
...
- Rendering and introduction UI use `image/ball/wormhole`; each endpoint renders at 80x80 in a dedicated zIndex 24 layer below the ball layer (zIndex 40), while cyclic arrows render in a separate zIndex 42 layer above the balls. Board rendering applies `effects/WormholeFlow`, and the texture is excluded from dynamic-atlas packing so its distortion UV domain remains stable. The cyclic channel contains only strict interior coordinates. Remote compact packs encode wormholes with type code `h` plus the explicit move direction.
