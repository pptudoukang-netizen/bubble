...
- 签到周期：`7` 天
- 每天可领取 `1` 次奖励
- 当天未领取时，可点击普通领取按钮领取 `1` 倍奖励，也可点击 `btn_award_ad` 看激励视频领取 `2` 倍奖励
- `btn_award_ad` 必须先完整观看广告，广告完成后才推进当天签到状态并发放双倍奖励
- 若 `GameBootstrap.rewardedVideoAdUnitId` 为空，点击 `btn_award_ad` 只提示“暂时还没有广告可看哦”，不推进签到状态、不发放奖励
...
- 已领状态
- 普通领取按钮 `btn_award`
- 广告双倍领取按钮 `btn_award_ad`
- 第 7 天大奖强调

交互规则：

- `btn_award`：直接领取当天 `1` 倍签到奖励
- `btn_award_ad`：完整观看激励视频后领取当天 `2` 倍签到奖励
- 无广告配置时，`btn_award_ad` 保持可点击，但点击只提示“暂时还没有广告可看哦”
- 今日已领取后，`btn_award` 与 `btn_award_ad` 均不可继续领取
...
- 第七天领取后进入下一轮
- 漏签不重置进度
- 未领取当天点击 `btn_award_ad`，完整看完广告后发放双倍奖励并标记今日已领取
- 未领取当天点击 `btn_award_ad`，广告提前关闭或展示失败时不发奖、不标记今日已领取
- `GameBootstrap.rewardedVideoAdUnitId` 为空时点击 `btn_award_ad`，提示“暂时还没有广告可看哦”，不发奖、不标记今日已领取
